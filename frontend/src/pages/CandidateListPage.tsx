...
export function CandidateListPage({ onOpenDetail }: CandidateListPageProps) {
  const { data, loading, error } = useQuery<{ candidates: CandidateItem[] }>(GET_CANDIDATES);
  ...
  const items = useMemo(() => {
    ...
    return source.filter((candidate: CandidateItem) => {
    ...
    <main className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      <section className="rounded-4xl border border-line bg-white/80 p-6 shadow-soft backdrop-blur xl:p-8 dark:border-white/10 dark:bg-white/5">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-accent-500">Candidate board</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-ink-950 dark:text-white">候选人管理面板</h2>
            <p className="mt-2 text-sm text-ink-500 dark:text-slate-300">查看候选人状态、技能标签和岗位评分，支持表格与卡片双视图。</p>
          </div>
          <div className="grid gap-3 lg:min-w-[24rem]">
            <input
              className="w-full rounded-2xl border border-line bg-white px-4 py-3 text-sm text-ink-900 outline-none transition placeholder:text-ink-400 focus:border-accent-300 focus:ring-4 focus:ring-accent-100 dark:border-white/10 dark:bg-slate-950/60 dark:text-white"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="搜索姓名、技能、邮箱"
            />
            <TabGroup selectedIndex={view === 'table' ? 0 : 1} onChange={(index) => setView(index === 0 ? 'table' : 'card')}>
              <TabList className="inline-flex w-fit rounded-full border border-line bg-white/70 p-1 dark:border-white/10 dark:bg-white/5">
                {['表格', '卡片'].map((label) => (
                  <Tab
                    key={label}
                    className="rounded-full px-4 py-2 text-sm font-medium text-ink-600 outline-none transition data-[selected]:bg-ink-950 data-[selected]:text-white dark:text-slate-300"
                  >
                    {label}
                  </Tab>
                ))}
        ...
        {loading ? <div className="mt-6 rounded-3xl bg-slate-100 px-4 py-4 text-sm text-ink-600 dark:bg-slate-900/60 dark:text-slate-300">正在加载候选人...</div> : null}
        {error ? <div className="mt-6 rounded-3xl bg-danger-100 px-4 py-4 text-sm text-danger-700">{error.message}</div> : null}
        {!loading && !error && items.length === 0 ? <div className="mt-6 rounded-3xl border border-dashed border-line-strong px-4 py-5 text-sm text-ink-500 dark:border-white/10 dark:text-slate-300">没有匹配的候选人。</div> : null}
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {items.map((candidate: CandidateItem) => (
              <button
                key={candidate.id}
                type="button"
                className="grid gap-4 rounded-[1.75rem] border border-line bg-white/85 p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-accent-300 dark:border-white/10 dark:bg-slate-950/40"
