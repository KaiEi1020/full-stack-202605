...
系统 MUST 在当前实现中根据模块复杂度，通过 REST 控制器、应用服务、领域边界、基础设施实现或直接 CRUD 服务装配后端能力。在单体应用顶层结构中，业务模块 MUST 归入 `src/modules/`，系统级支撑模块 MUST 归入 `src/core/`。
...
系统 MUST 保证复杂模块中的依赖方向清晰，并统一通过 TypeORM 访问持久化数据；简单模块 MUST 保持控制器到服务到持久化访问的单向依赖。系统级基础设施能力 MUST 作为被业务模块依赖的支撑模块组织在 `src/core/` 下，而不是与业务模块并列混放。
